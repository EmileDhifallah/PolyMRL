from helpers_from_edm import utils
import numpy as np
import math
import torch
model import models
from torch.nn import functional as F
from helpers_from_edm import utils as diffusion_utils
...
# this is the model that is being ran within the training loop; it is a wrapper of the entire diffusion pipeline including the backwards
# diffusion process through the EGNN (wrapped by its EGNN_dynamics model class).
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        '''
        inputs into initializing EnVariationalDiffusion:
        dynamics: the graph neural network class that wraps around the (E)GNN
        in_node_nf: length of vocabulary; used for determining Gaussian noise sample (e.g. 4 for QM9 dataset);
        n_dims: coord num features, i.e. 3 for (3D molecule) coordinates;
        timesteps: I believe the nr. of denoising steps to go back to a (hopefully) realistic looking molecule;
        ..
        loss_type: type of training loss signal? Here, variational lower bound (I thought this was L2 in practice..?) is chosen;
        norm_values: ...
        norm_biases: ...
        include_charges: whether we use charge values of atoms during training or not (the default for qm9 here is yes).
        ...
        if noise_schedule == 'learned':
            self.gamma = GammaNetwork() # ah we can apparently learn the forward noise here using something called a Gamma network!
        else:
...
# note: net_out is (our) 'eps_pred'
    def compute_error(self, net_out, gamma_t, eps):
        ...
        if self.training and self.loss_type == 'l2':
            ...
            error = sum_except_batch((eps - eps_t) ** 2) / denom # sum_except_batch aggregates the seq_len and emb dimensions, and sums over these two; one loss value per molecule is the result
        else:
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        # note: we actually concatenate the node type information (.. and charges..?) with the atom coordinates x, which means we noise these as well; does that mean we add noise to a one-hot encoded vector..? --> yes, it does (I think..?)
        ...
        # check if the node feature mask is correct (not sure what it does exactly)
        diffusion_utils.assert_mean_zero_with_mask(z_t[:, :, :self.n_dims], node_mask)

        # Neural net prediction.
        print('inputs into EGNN Dynamics, including noised coordinates+node features: ')
        print('z_t ', z_t, z_t.shape)
        print('t ', t,)
        print('node_mask: ', node_mask, node_mask.shape)
        print('edge_mask: ', edge_mask, edge_mask.shape)
        print('context: ', context)

        # get net_out, which is concat([vel, h]), with h final node repr. in shape (batch, seq_len, h_num_feats), and
        # vel (batch, seq_len, 3); like the inputs of the two;
        net_out = self.phi(z_t, t, node_mask, edge_mask, context)
        # note: this output is the concatenation of [vel] with the final node representation [h_final],
        # where vel is the predicted noise "eps" added to the original molecule in this compute_loss function;
        # of (batch, seq_len, 3(x)+6(h))

        print('output of EGNN Dynamics: ', net_out, net_out.shape)


        # Compute the error.
        
        # note: this is difference between the actual noise (added to z_t), and the predicted noise by the EGNN (net_out), i.e. the l2 loss
        #  --> q: what is the shape of this loss (object)?
        print('eps: ', eps, eps.shape if isinstance(eps, torch.Tensor) else 'eps is not a tensor')

        error = self.compute_error(net_out, gamma_t, eps)
        print('error after dynamics function: ', error) # shape (batch, ) = (64, )

        # note: now, I believe, in order to get to the variational lower bound loss, which is what we want to use as the training signal,
        # we need to do reparameterization, for which we first, for notational ease of use, define the Signal to Noise Ratio (SNR), which
        # we use in the derivation/calculation below; therefore we first define the SNR value below
        ...
        print('final loss compute_loss: ', loss, {'t': t_int.squeeze(), 'loss_t': loss.squeeze(),
                      'error': error.squeeze()})
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        """
        # shapes here are exactly those that enter the compute_nll function that is called within the train_epoch() function;
        # now we need to track where it goes into egnn dynamics and then into egnn.


        # Normalize data (x and h), take into account volume change in x.
        x, h, delta_log_px = self.normalize(x, h, node_mask)
        print('x after normalizing: ', x, x.shape)
        print('h after normalizing: ', h)

        # Reset delta_log_px if not vlb objective.
        if self.training and self.loss_type == 'l2':
            delta_log_px = torch.zeros_like(delta_log_px)

        # note: the loss coming out of compute_loss here is already printed in the compute_loss() function above.
        if self.training:
            # Only 1 forward pass when t0_always is False.
            # note: I think t0_always refers to whether we start noising/denoising from timestep 0 or a random timestep; during training we want to randomize timesteps I believe.
        ...
        print('neg log pxh (final loss of model: )', neg_log_pxh)
        return neg_log_pxh
