...
def print_linear_stats(model, layer_name=""):
    """Print weight and bias statistics for linear layers"""
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            print(f"  Norm: {weight.norm():.6f}")
            # print(f"  Weight [:5]: {weight[:5]}")
            
            # Bias statistics (if exists)
            if module.bias is not None:
                bias = module.bias.data
                print(f"Bias: shape={bias.shape}")
                print(f"  Mean: {bias.mean():.6f}, Std: {bias.std():.6f}")
                print(f"  Min: {bias.min():.6f}, Max: {bias.max():.6f}")
                # print(f"  Values [:5]: {bias[:5]}")  # First 10 values
            else:
                print("Bias: None")
...
class polyMRL(nn.Module):
    def __init__(self, in_node_nf, context_node_nf,
        ...
        if self.train_mode=='property_prediction':
            if self.only_1d or self.only_3d:
                ...
                # cls_dim_nf=768 # I think: 1d is 768, 3d is 512, contrasted-1d- and -3d are both 512; the combined contrasted is 1024.
            ...
            # self.layer1 = nn.Linear(cls_dim_nf, 128)
            # self.act1 = nn.ReLU()
            ...
            # variant 2
            # self.dropout = nn.Dropout(0.1)
            # self.layer1 = nn.Linear(cls_dim_nf, 1028)
            # self.act1 = nn.ReLU()
            # self.dropout = nn.Dropout(0.1)
            ...
            # TODO: include unfreeze_layers here! -> not necessary, done in train_test.py
    ...
    def forward(self, combined_1d_3d): # smiles_tokens, smiles_attention_mask, x, h, node_mask, edge_mask, context):
        ...
        # comment ''' here to remove 3d encoding
        ## 3d
        ...
        # doing property prediction, compute MSE as loss
            ...
            if self.only_1d: # only 1d encoder's cls
                ...
                cls_representation = X_3d#torch.cat([X_1d, X_3d], dim=-1)
            ...
            with torch.no_grad():
                ...
                # cls_representation_testing = cls_representation_testing*35.
                # print(f"After *35.0 multiplier: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
            ...
            with torch.no_grad():
                print(f"Before dropout - 30.0: std: {(cls_representation*30.0).std():.4f}, mean: {(cls_representation*30.0).mean():.4f}, norm: {(cls_representation*30.0).norm():.4f}")
                cls_representation_testing = self.dropout(30.0*cls_representation)
                print(f"After dropout - 30.0: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
                cls_representation_testing = self.layer1(cls_representation_testing)
                print(f"After linear 1 - 30.0: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
                # cls_representation_testing = self.act1(cls_representation_testing)
                # print(f"After activation - 30.0: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
                # cls_representation_testing = self.layer2(cls_representation_testing)
                # print(f"After linear 2 - 30.0: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
                # cls_representation_testing = self.act2(cls_representation_testing)
                # print(f"After activation 2 - 30.0: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
                # cls_representation_testing = self.layer3(cls_representation_testing)
                # print(f"After linear 3 - 30.0: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
            with torch.no_grad():
                print(f"Before recenter - 30.0 with mean scaling beforehand: std: {(cls_representation).std():.4f}, mean: {(cls_representation).mean():.4f}, norm: {(cls_representation).norm():.4f}")
                cls_representation_testing = cls_representation - cls_representation.mean(dim=1, keepdim=True)
                print(f"Before scaling - 30.0 with mean scaling beforehand: std: {(cls_representation_testing).std():.4f}, mean: {(cls_representation_testing).mean():.4f}, norm: {(cls_representation_testing).norm():.4f}")
                print(f"Before dropout - 30.0 with mean scaling beforehand: std: {(cls_representation_testing*30.0).std():.4f}, mean: {(cls_representation_testing*30.0).mean():.4f}, norm: {(cls_representation_testing*30.0).norm():.4f}")
                cls_representation_testing = self.dropout(30.0*cls_representation)
                print(f"After dropout - 30.0 with mean scaling beforehand: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
                cls_representation_testing = self.layer1(cls_representation_testing)
                print(f"After linear 1 - 30.0 with mean scaling beforehand: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
                # cls_representation_testing = self.act1(cls_representation_testing)
                # print(f"After activation - 30.0: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
                # cls_representation_testing = self.layer2(cls_representation_testing)
                # print(f"After linear 2 - 30.0: std: {cls_representation_testing.std():.4f}, mean: {cls_representation_testing.mean():.4f}, norm: {cls_representation_testing.norm():.4f}")
            ...
            cls_representation = cls_representation - cls_representation.mean(dim=1, keepdim=True)  
            print('cls repr. after mean re-center:', cls_representation.std(), cls_representation.mean(), cls_representation.norm())#, cls_representation)
            cls_representation = 30.0 * cls_representation # 
            print('cls repr. after rescaling by 20.:', cls_representation.std(), cls_representation.mean(), cls_representation.norm())#, cls_representation)
            ...
            print_linear_stats(self.class_head, "class. Head")
            # print_linear_stats(model.classification_head, "Classification Head")

            prediction = self.class_head(cls_representation) # prediction is now also of shape (batch, seq_len, 1)
            print('after class head repr.:', prediction.std(), prediction.mean(), prediction.norm())


            # prediction = prediction * 35. # prediction is now also of shape (batch, seq_len, 1)
            # print('prediction after 35.0 multipl.:', prediction.std(), prediction.mean(), prediction.norm())
