...
class EGNN_dynamics_PI1M(nn.Module):
...
# structure from mm_polymer; simply linear -> act -> linear, to some desired out dim.
class NonLinearHead(nn.Module):
    """Head for simple regression (or classification with last layer adjusted to some n_classes) tasks."""
...
class PolyMRL(nn.Module):
    def __init__(self, in_node_nf, context_node_nf,
                 ...
                 use_layernorm=True, only_1d=False, only_3d=False, scaling_value=1.0, oligomer=False
                 ):
        '''
        
                
        The model class has a forward module that contains, on one hand a call to the (dynamics function of the) EGNN module, and on the other hand,
        a call to the language model, as well as a call to a module that returns cross-aligned representations of the two. After computing the 3 outputs,
        the resulting representations are used to compute the 3 loss values, summing these into one training signal; this is passed as output to the trainer;
        alternatively the downstream loss is passed as output in the case where a classification head is initialized ('property_predicton' mode).

        The loss functions used by default are L2 (EGNN), Binary Cross Entropy ('Language Modelling Loss' in the LM), and InfoNCE loss (Cross-Alignment).
        For property prediction, the default is Huber Loss, which protects better against outliers than e.g. basic Mean Squared Error.
        
        In the initialization module, the inputs consist of hyperparameters and shapes of the two submodules (EGNN & LM), which
        are passed to the model instance in the training script.
        ...
        ### initialize language model, using from_pretrained
        # wrong base, Roberta ZINC base model used by default
        ...
        self.net_dynamics = EGNN_dynamics_PI1M(  # TODO: rename to "net_3d"?
        ...
        # this goes from a small dim. (6) to high dim. (512 for example)
        self.contr_3d_layer = NonLinearHead( # another intermediate feedforward layer going from dim. 512 to 512
            self.in_node_nf, 512, act_fn
        )
        
        # 1d feedforward layer goes from output dim. 512 to 512
        self.contr_3d_layer_hidden = NonLinearHead( 
            self.hidden_nf, 512, act_fn
        )

        # alternative usage of 3d output, using the combined x+h reprs. (not the default)
        ...
        # loading in 'warmed up' encoders if selected in input parameters (encoders trained separately for 1 epoch to start with meaningful weights for their respective modality)
        ...
        ### initializing downstream head, if running property prediction
        self.class_head=None
        if self.train_mode=='property_prediction':
            if self.only_1d or self.only_3d:
                cls_dim_nf = 512
            else:
                # by default, shapes are 768 for 1d, in_node_nf for 3d (length of atom vocabulary which is approx. 20 for us), contrasted 1d and 3d reprs. are both 512; concatenated contrasted repr. is then 1024.
                # cls_dim_nf=768 (1d)
                # cls_dim_nf=self.in_node_nf (3d)
                # cls_dim_nf = 1024
            ...
            # regression head, projecting down from the concatenated CLS tokens to one floating-point target
            # variant 1  (2 layers)
            ...
            # extra projection layers, separate from the aligned projection layers, to project 1d & 3d to the same dimensionality
            self.equal_layer_1d = nn.Linear(768, 512)
            self.equal_layer_3d = nn.Linear(self.in_node_nf, 512)

            self.pred_loss = torch.nn.HuberLoss() # original: MSELoss()
            # self.r2_metric = R2Score()
            
            # if we are using oligomers, we want to test on an empty checkpoint for comparability!
            if not self.oligomer:
                print('loading checkpoint;')
                # load in pretrained weights of the encoders and projection heads:
                # -> note: currently we ignore h-related weights because of mismatch; two options to improve this are 1) manual setting of pretrained h-weights (using e.g. weight[:,:,:20]); 2) creating one 'master-vocabulary' over all datasets, and training all datasets using this atom vocabulary directly;
            ...
            # learned scaling parameters to get range of 1d and 3d output weights closer together
            ...
            # note: unfreezing using unfreeze_layers is implemented in the trainer

    # functions for freezing and unfreezing of encoders
    ...
    def compute_error(self, net_out, eps):
        ...
        eps_t = net_out # shape (batch, seq_len, h+x) = (64, 25, 20+3)
        diffs = eps - eps_t
    ...
    def contrastive_loss(self, X_1d, X_3d, temperature=0.07):
        """
        X_1d: (B, 1d_out_dim) - LM repr.
        X_3d: (B, 3d_out_dim) - EGNN repr.
        """
        B = X_1d.shape[0]

        # define cosine similarity matrix of shape (B, B)
        sim_matrix = torch.matmul(X_1d, X_3d.T) / temperature

        # define ground truth labels; diagonal elements are positives
    ...
    def sample_combined_position_feature_noise(self, n_samples, n_nodes, node_mask):
        ...
        """
        z_x = sample_center_gravity_zero_gaussian_with_mask( # func. helpers_from_edm/utils
            size=(n_samples, n_nodes, self.n_dims), device=node_mask.device,
            node_mask=node_mask)
        z_h = sample_gaussian_with_mask( # func. helpers_from_edm/utils
    ...
    def forward(self, combined_1d_3d): # smiles_tokens, smiles_attention_mask, x, h, node_mask, edge_mask, context):
        ...
        # (unpacked: seq_rep_output.loss, seq_rep_output.logits, seq_rep_output.hidden_states, seq_rep_output.attentions)

        ## 3d
        # (s) comment ''' here to isolate 1d encoding (i.e. remove 3d part)
        ...
        # noising functionality for xh using 'eps' and sigma/alpha values
        # learned noise level using Gamma network (from DDPM method)
        if self.learned_noise:
            # compute gamma via the network, using a randomly sampled "timestep" (taken from DDPM methodology)
            gamma = self.inflate_batch_array(self.gamma(np.random.randn()), x)

            # get sigma and alpha tensors in shape of x
            sigma = self.sigma(gamma, x).to(self.device)
            alpha = self.alpha(gamma, x).to(self.device)

        # fixed noise level for sigma/alpha
        else:
            ...
            sigma_value = 0.05
            sigma_value = self.inflate_batch_array(torch.tensor([sigma_value], device=self.device), x) # tensor([[[0.05]]])
            
            # get sigma and alpha tensors in shape of x
            sigma = self.sigma_fixed_schedule(sigma_value, x) # tensor([[[0.05]]])
            alpha = self.alpha_fixed_schedule(sigma_value, x) # tensor([[[0.9987]]])
        ...
        # eps_old = eps.clone().detach() # keep a copy of the noise var for checking the mask+noise
        ...
        print('shape of combined tensor xh: ', xh.shape) # (64, 25, 23) = (batch, seqlen, x+h)
        ...
        # -> the output 'eps_pred' is cat(vel, h_output), i.e. the output of EGNN_dynamics_PI1M()'s forward
        ...
        X_a_L = h_final # final atom repr. (of which we take atom 0 3d representation or an aggregated version; shape = (batch, num_atoms/seq_len, len(atom_vocab))
        X_a_L = X_a_L * node_mask
        # (e) comment ''' here to remove 3d encoding
        ...
        print('seq_rep_output.hidden_states mean: ', seq_rep_output.hidden_states[-1].mean().item())
        ...
        if self.cls_rep_3d=='virtual_atom':
            ...
            # old: X_3d = X_a_L.mean(dim=1) # = (batch, 20)
        ...
        # take the first real atom of the sequence (i.e. not the virtual (empty) atom)
        ...
        # take all atoms' reprs. besides the CLS token's (we exclude that) and aggregate
        ...
        # rescale 3d repr. (decide if only to use for property prediction)
        X_3d = 4.0 * X_3d
        print('x3d std after rescale * 4.0: ', X_3d.std().item()) # 0.15 start of epoch and increases to 0.65 at the end of the epoch
        
        # # X_1d = self.equal_layer_1d(X_1d)
        # # X_3d = self.equal_layer_3d(X_3d)
        ...
        if self.train_mode=='pretrain':
            ...
            # (we only project during pre-train and not during prop. pred. as the encoders are aligned already during pre-training)
            if X_a_L.shape[2]>50:
            ...
            print('X_3d_projected std: ', X_3d_projected.std().item())

            # normalize projected representations using their mean
            # X_1d_projected = F.normalize(X_1d_projected, dim=-1)
            # X_3d_projected = F.normalize(X_3d_projected, dim=-1)

            # compute 3D i.e. denoising (l2) error (note: if we use virtual atoms, the 0-th atoms are excluded from this denoising noise computation)
            ...
            # weights chosen empirically during pre-training
        ...
        # doing property prediction, using Huber or MSE as loss
        elif self.class_head:
            targets = batch_3d['target'] # target is of shape (batch, 1)
            
            # we need mad and mean of train dataloader for target normalization
            mad = batch_3d['mad']
            mean = batch_3d['mean']
            print(f'mean / mad: {mean} / {mad}')

            # create (concatenated) cls representation
            if self.only_1d: # only 1d encoder's cls
                print('just 1d')
                cls_representation = X_1d
            elif self.only_3d: # only 3d encoder's cls
                print('just 3d')
                cls_representation = X_3d
            else: # combining both modalities
                print('combined 1d-3d')
                cls_representation = torch.cat([X_1d, X_3d], dim=-1) # X_3d
                # cls_representation = torch.cat([X_1d_projected, X_3d_projected], dim=-1)

            # logging magnitudes of the classification layers
            with torch.no_grad():
                cls_representation_testing = self.dropout(5.0*cls_representation.clone().detach())
            ...
            print('rescaled CLS repr. std-mean-norm:', cls_representation.std(), cls_representation.mean(), cls_representation.norm())#, cls_representation)

            if self.use_layernorm:
                print('applying class. layernorm')
                cls_representation = self.layernorm_class(cls_representation) # should bring cls to mean 0, std 1 (instead of +=std=0.05 that comes out of encoded reprs.) (value consistently around 0.994 - 0.997)
            else:
                print('not applying class. layernorm (=default behavior)')

            prediction = self.class_head(cls_representation)

            # add learned scaling parameter
            # prediction = self.output_scale * prediction + self.output_shift
            # print('learned output_scale: ', self.output_scale)
            # print('learned output_shift: ', self.output_shift)
            ...
            if self.partition=='train':
                ...
                if len(prediction.shape)>1:
                    ...
                    print('prediction tensor was squeezed, new shape: ', prediction.shape)
                
                if len(targets.shape)>1:
                    targets=targets.squeeze()
                    print('target tensor was squeezed, new shape: ', targets.shape)
                ...
                print('predictions scaled to target/test dist.: ', (mad*prediction+mean).std(), (mad*prediction+mean).mean(), (mad*prediction+mean)[:5])

                # include drift penalty term
                # drift_penalty = compute_drift_penalty(self.net_1d)

                # note: this is by default the Huber loss value - however we log and evaluate on the RMSE and R squared values still too
                ...
                if len(prediction.shape)>1:
                    prediction=prediction.squeeze()
                    print('squeezed original prediction tensor, new shape: ', prediction.shape)

                if len(targets.shape)>1:
                    targets=targets.squeeze()
                    print('squeezed targets tensor, new shape: ',targets.shape)
                ...
                print('targets: ',targets.std(), targets.mean(), targets[:5])
                print('targets scaled to training dist.: ', ((targets-mean)/mad).std(), ((targets-mean)/mad).mean(), ((targets-mean)/mad)[:5])
                ...
                print('First 3 (predictions, target) pairs:')
                count=0
