...
from pi1m.analyze import check_stability
...
def sample_chain(args, device, flow, n_tries, dataset_info, prop_dist=None):
    ...
    if args.probabilistic_model == 'diffusion':
        ...
        for i in range(n_tries):
            ...
            chain = torch.cat([chain, chain[-1:].repeat(10, 1, 1)], dim=0) # what is meant by a chain? Isnt it a single molecule that is sampled? Is it maybe the biggest connected component of all the generated atoms/groups of atoms?
            x = chain[-1:, :, 0:3]
            one_hot = chain[-1:, :, 3:-1]
            one_hot = torch.argmax(one_hot, dim=2)

            atom_type = one_hot.squeeze(0).cpu().detach().numpy() # get one hot encoding and then atom types from sampled chain I think ..?
            x_squeeze = x.squeeze(0).cpu().detach().numpy()
            mol_stable = check_stability(x_squeeze, atom_type, dataset_info)[0] # get molecular stability score of the generated molecule

            # Prepare entire chain.
            x = chain[:, :, 0:3]
            one_hot = chain[:, :, 3:-1]
            one_hot = F.one_hot(torch.argmax(one_hot, dim=2), num_classes=len(dataset_info['atom_decoder']))
            charges = torch.round(chain[:, :, -1:]).long() # atomic charges are also generated/predicted it seems?
