...
from pi1m.data.prepare.process import process_xyz_files, process_xyz_gdb9, process_mm_lmdb_files
from pi1m.data.prepare.utils import download_data, is_int, cleanup_file


def download_dataset_qm9(datadir, dataname, splits=None, calculate_thermo=False, exclude=True, cleanup=True):
    """
    Download and prepare the QM9 (GDB9) dataset.
    """
    print('download_dataset_qm9 called')
    ...
    # gdb9_tar_data = urllib.request.urlretrieve(gdb9_url_data, filename=gdb9_tar_data)
    ...
    print('os.listdir(datadir): ', os.listdir(datadir))
    print('gdb9_tar_data: ', gdb9_tar_data)

    # note: added myself
    # check if we have a lmdb train/valid/test split as datadir
    if any([filepath.endswith(".lmdb") for filepath in os.listdir(datadir)]) and len(os.listdir(datadir))>1:
        splits = {'train':False, 'valid':False, 'test':False}

        for filename in os.listdir(datadir):
            if filename.endswith("train.lmdb"):
                splits['train'] = os.path.join(datadir, filename)
            if filename.endswith("valid.lmdb"):
                splits['valid'] = os.path.join(datadir, filename)
            if filename.endswith("test.lmdb"):
                splits['test'] = os.path.join(datadir, filename)                
            # should now have, for example: splits = {'train': './data/testing_data/Eat/train.lmdb', 'valid': './data/testing_data/Eat/valid.lmdb', 'test': './data/testing_data/Eat/test.lmdb'}
    print('splits: ', splits)
    # print('splits 1: ', splits) # these are actually the indices for all of the train/valid/test splits in one dict {train: [300,405,...],...}; not the dataset itself. So figure out where to place our .lmdb files.!!
    # print('splits 1 items: ', splits.items() if isinstance(splits, dict) else None)

    # If splits are not specified, automatically generate them.
    # note; generate data splits
    if splits is None:
        splits = gen_splits_gdb9(gdb9dir, cleanup)

    # print('splits 2: ', splits) # these are actually the indices for all of the train/valid/test splits in one dict {train: [300,405,...],...}; not the dataset itself. So figure out where to place our .lmdb files.!!
    # print('splits 2 items: ', splits.items())

    # -> NOTE: this is not the location where we intervene since splits contains split idxs, and not the data itself; we must find the location where the data itself is loaded in to insert our .lmdb data

    # note intervene HERE, by loading in lmdb dataset train/valid and test sets, inside of a dictionary with the same features as splits.items() here
    # one thing to-do: check if the 'splits' variable here is identical to the 'datasets as input' output; if so, simply insert our lmdb files in the utils.py script instead!
    # note 2: do note that then we do not make use of the .npz file format, but rather directly load in our lmdb data files and use them into our pipeline from there;

    # Process GDB9 dataset, and return dictionary of splits
    # so we get gdb9_data = {'train': {molecules train dict}, 'valid': {molecules valid dict}, 'test': {molecules test dict}}
    gdb9_data = {}
    
    # NOTE we modified this, since split_path was split_idx before, and process_xyz_files had as 1st argument 'datadir' I believe.
    if isinstance(splits['train'], str) and splits['train'].endswith('.lmdb'):
        
        for split, split_path in splits.items(): # train: train_path for us! -> split_path was split_idx
            print('split,split_path', split,split_path)
            gdb9_data[split] = process_xyz_files(
                split_path, process_mm_lmdb_files, file_idx_list=None, stack=True) # orig: gdb9_tar_data ipv lmdb_data_paths and process_xyz_gdb9 ipv process_mm_lmdb_files
    else:
        for split, split_idx in splits.items(): # train: train_path for us! -> split_path was split_idx
            print('split,split_idx', split,split_idx)
            gdb9_data[split] = process_xyz_files(
                gdb9_tar_data, process_xyz_gdb9, file_idx_list=split_idx, stack=True) # orig: gdb9_tar_data ipv lmdb_data_paths and process_xyz_gdb9 ipv process_mm_lmdb_files
    ...
    # note: this is the .npz file, which is printed as 'datasets as input: ' in the .out log;
...
def get_thermo_dict(gdb9dir, cleanup=True):
    ...
    # Dictionary that transforms atom type/element number to the atom charge
