...
from pi1m.data.prepare.md17 import download_dataset_md17
from pi1m.data.prepare.qm9 import download_dataset_qm9
...
def prepare_dataset(datadir, dataset, subset=None, splits=None, cleanup=True, force_download=False):
    ...
    # get paths to existing data .npz files
    ...
    if new_download or force_download:
        ...
        if dataset.lower().startswith('qm9'):
            # note: this is only called if the dataset is not downloaded already
            # note 2: this function does not return anything! only downloads train/valid/test to .npz files in passed data directory path
