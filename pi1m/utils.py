...
def prepare_context(conditioning, minibatch, property_norms):
    '''
    this does 3 things:
    take all keys from conditioning that we want as properties, and subtract their means (and mad) to normalize the entries
    repeat the property per molecule for every atom in the molecule
    concatenate all condition tensors over the hidden dimension to create one (batch, seq_len, context_nf)-shaped tensor that we pass to the model
    '''
