#!/bin/bash
#SBATCH --time=1:00:00
#SBATCH --job-name=prediction_oligomers
#SBATCH --output=/home/14044994/test/Repos/PolyMRL/script_outputs/predicting_ol_pi1m_out_%j-oligomers.log
#SBATCH --error=/home/14044994/test/Repos/PolyMRL/script_outputs/predicting_ol_pi1m_err_%j-oligomers.log
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
for data in "${data_list[@]}"
        do
                data_direc="pi1m/mmpolymer_oligomer_data/${data}" # replace to your data path
                data_name="${data}"
                experi_name="${data}"

                python -m main_pi1m --datadir $data_direc --dataset $data_name --mixed_precision_training False --amp_dtype 32 --lr 8e-3 --weight_decay 0.00005 --oligomer False --pretrained_dir outputs/final_model_combined_PI1M_no-amp_loaded-PI1M-noampfp32-encoders_epoch_3 --train_mode property_prediction --scaling_value 2.0 --only_1d False --only_3d False --variable_lrs True --unfreeze_encoders True --unfreeze_projections True --freeze_encoders True --freeze_projections True --n_epochs 3001 --batch_size 16 --exp_name $experi_name-test-repo-3001ep --cls_rep_3d mean_pool --reflection_equiv True --n_stability_samples 1000 --wandb_usr emiledhifallah --diffusion_noise_schedule polynomial_2 --diffusion_noise_precision 1e-5 --diffusion_steps 1000 --diffusion_loss_type l2 --nf 256 --n_layers 9 --normalize_factors [1,4,10] --test_epochs 1 --ema_decay 0.9999
        done
...
# combined pretrained checkpoint: final_model_combined_PI1M_no-amp_loaded-PI1M-amp-encoders_epoch_3 
#       OR 
#        final_model_combined_PI1M_no-amp_loaded-PI1M-noampfp32-encoders_epoch_3
# combined expname --exp_name $experi_name-TESTING-GRID-long3mlp-huberloss-slowlyunfrozen-unfrozenprojs-lowwd-targetnormal-1000ep-noproj-bs64-highlr-combinedcheckpoint-scale30-3dscale4-nonormalize
